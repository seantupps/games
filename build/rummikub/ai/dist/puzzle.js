...
function buildPuzzle(pool, melds, target, strictConnect) {
    if (melds.some((m) => !meldSizeValid(m)))
        return null;
    if (strictConnect && boardHasConnectablePair(melds))
        return null;
    const grid = new Grid();
 ...
 */
export function generateSolvedBoard(rng, placedCount = 53, variant = 'standard', timeoutMs = DEFAULT_TIMEOUT_MS) {
    const t0 = Date.now();
    ...
    while (Date.now() < deadline) {
        ...
        const { melds, placed, remaining } = fullBoard
            ...
            : { ...greedyFill(work, target, rng, deadline, false), remaining: [] };
        if (fullBoard) {
        ...
        if (!puzzle || puzzle.placedCount !== target)
            continue;
        return {
            ok: true,
            timedOut: false,
...
export function demoPuzzle() {
    ...
    const rack = [tile('B', 4), tile('O', 11), tile('U', 3), tile('R', 12)];
    return {
        grid,
        rack: sortRack(rack),
        melds: [runU, runR],
