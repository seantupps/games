...
function parseArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        else if (a === '--placed' && argv[i + 1])
            out.placed = parseInt(argv[++i], 10);
        else if (a === '--remove' && argv[i + 1])
            out.remove = parseFloat(argv[++i]);
...
function validatePlaced(placed, variant) {
...
function validateRemove(remove) {
...
function runSeed(baseSeed, runIndex) {
    if (baseSeed != null)
        return (baseSeed + runIndex) >>> 0;
    return (Math.floor(Math.random() * 1_000_001) + runIndex) >>> 0;
}
function genTimeoutSec(placed, variant, timeout) {
    if (timeout <= 0)
        return Number.MAX_SAFE_INTEGER / 1000;
    if (placed >= expectedPoolSize(variant) - 1)
        return Math.max(timeout, FULL_BOARD_TIMEOUT_SEC);
    return timeout;
}
function playOneRun(args, placed, seed, log) {
