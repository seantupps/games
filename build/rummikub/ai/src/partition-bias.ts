...
export function buildOriginalLongRuns(melds: Meld[], minLen = LONG_RUN_MIN): OriginalLongRun[] {
  const out: OriginalLongRun[] = [];
...
function tileLongRunMembership(tileId: string, runs: OriginalLongRun[]): number {
  let n = 0;
...
function isContiguousOriginalSubRun(meld: Meld, orig: OriginalLongRun): boolean {
...
function contiguousOriginalMatch(meld: Meld, runs: OriginalLongRun[]): OriginalLongRun | null {
  for (const run of runs) {
...
export function meldPickWeight(meld: Meld, runs: OriginalLongRun[]): number {
...
export function partitionPreservationScore(melds: Meld[], runs: OriginalLongRun[]): number {
  let score = 0;
  for (const meld of melds) {
...
export function weightedPickMelds<T extends Meld>(items: T[], weightFn: (m: T) => number, rng: { randrange(n: number): number }): T {
  let total = 0;
