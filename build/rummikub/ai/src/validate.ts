...
export function assignJokersToMeld(meld: Meld): { color: Color; value: number }[] | null {
  return assignJokers(meld.tiles, meld.kind) as { color: Color; value: number }[] | null;
}

function assignJokers(tiles: Tile[], kind: 'group' | 'run'): Resolved[] | null {
  const resolved: Resolved[] = new Array(tiles.length);
  ...
  const anchors = resolved
    ...
    .filter(Boolean) as { i: number; color: Color; value: number }[];
...
function validateGroupResolved(resolved: Resolved[]): boolean {
...
function validateRunResolved(resolved: Resolved[]): boolean {
...
export function materializeMeld(meld: Meld): Meld {
  const assigned = assignJokers(meld.tiles, meld.kind);
  if (!assigned) return meld;
  const tiles: Tile[] = meld.tiles.map((t, i) => {
    if (t.kind === 'joker') {
      const a = assigned[i]!;
...
export function scoreTilesFromRack(tiles: Tile[]): number {
