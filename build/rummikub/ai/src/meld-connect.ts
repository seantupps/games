...
export function summarizeMeld(meld: Meld): MeldSummary | null {
...
export function meldsCanConnect(a: MeldSummary, b: MeldSummary): boolean {
...
export function canConnectToAny(candidate: Meld, existing: Meld[]): boolean {
...
export function boardHasConnectablePair(melds: Meld[]): boolean {
  const summaries = melds.map(summarizeMeld).filter(Boolean) as MeldSummary[];
