...
async function assertGuestFirstSplitStableAfterReset(hostPage, guestPage, lib, options = {}) {
    ...
    if (!preTouch.tileCount || preTouch.gameStarted) {
        throw new Error(`${label}: expected face-down pre-SPLIT hand (${JSON.stringify(preTouch)})`);
    }
    ...
    if (!touch.ok || !touch.gameStarted) {
        throw new Error(`${label}: guest touch did not start game (${JSON.stringify(touch)})`);
    }
    await flushHostBananaInteractions(hostPage).catch(() => {});
    ...
    if (!baseline.modelSig) {
        throw new Error(`${label}: no tile letters after guest touch`);
    }
    ...
    while (Date.now() - started < WATCH_MS) {
        ...
        if (snap.modelSig !== baseline.modelSig) {
            throw new Error(
                `${label}: tile letters changed in model at ${snap.atMs}ms\n`
                + `before=${baseline.modelSig}\n`
                + `after=${snap.modelSig}\n`
                + `samples=${JSON.stringify(samples.slice(-6))}`
            );
        }
        ...
        for (const t of baseline.model) {
            ...
            if (domLetter && domLetter !== t.letter) {
                throw new Error(
                    `${label}: DOM letter changed for ${t.id} at ${snap.atMs}ms `
                    + `(expected ${t.letter}, got ${domLetter})`
                );
            }
        }
    }
