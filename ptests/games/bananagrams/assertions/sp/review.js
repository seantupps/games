...
async function assertReviewShowsPlayerBoard(gameFrame, snapshot, label) {
    ...
    if (!state.ok) {
        failWithSnapshot('assertion', [`${label}: review must show your board (${JSON.stringify(state)})`], {});
    }
}
...
async function clickDone(gameFrame) {
    await gameFrame.evaluate(() => {
        const g = window.game;
        const btn = document.getElementById('banana-done-btn');
        if (!btn?.classList.contains('show')) failWithSnapshot('assertion', [String('Done button not visible')], {});
...
async function testBananasDevWinDoneTwice(page, gameFrame, opts = {}) {
    ...
    for (let round = 1; round <= 2; round++) {
        ...
        if (!setup.ok) {
            failWithSnapshot('assertion', [`Round ${round}: no tiles to snapshot (${JSON.stringify(setup)})`], {});
        }
        ...
        if (review.banner) {
            failWithSnapshot('assertion', [`Round ${round}: no in-game win banner (${JSON.stringify(review)})`], {});
        }
...
async function testBananasVictoryDone(page, gameFrame, opts = {}) {
    ...
    if (!setup.ok) {
        failWithSnapshot('assertion', [`Win board setup failed (${JSON.stringify(setup)})`], {});
    }
    ...
    if (!win.peeled || win.banner) {
        failWithSnapshot('assertion', [`Expected peel win without BANANAS banner (${JSON.stringify(win)})`], {});
    }
    if (!win.isOver || !win.victoryRegistered || !win.postGameReview || !win.timerFrozen) {
        failWithSnapshot('assertion', [`Victory / review / timer freeze not entered (${JSON.stringify(win)})`], {});
    }
    ...
    if (!hubBanner.visible || !looksLikeTimer(hubBanner.text)) {
        failWithSnapshot('assertion', [`Solo hub banner should show elapsed time (${JSON.stringify(hubBanner)})`], {});
    }
