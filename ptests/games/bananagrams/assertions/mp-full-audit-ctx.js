...
const { readDealDiag } = require('./mp-board-visible');
const { assertAllPlayersScoreboard } = require('./mp-scoreboard');
const { assertAllPlayersPoolSynced } = require('./mp-sync-board');
...
async function assertAllPlayersRefreshPreservesLayout(ctx, frames, tileIdByUid) {
    ...
    for (const player of order) {
        ...
        if (!tileId) throw new Error(`Missing tile id for refresh test (${player.role})`);
...
async function assertAllPlayersDragLocal(ctx, frames, opts = {}) {
    ...
    for (let i = 0; i < ctx.remotes.length; i++) {
        ...
        if (!remoteDrag.ok) {
            throw new Error(`${remote.role} drag failed (${JSON.stringify(remoteDrag)})`);
        }
...
async function runPostGameReviewAudit(ctx, frames, opts = {}) {
    const { captureEndingLayoutFromFrame } = require('./mp-review');
    const { runBananagramsMpMobilePostGame } = require('../scenarios/mp/review-mobile');
    const { assertHostSplitSyncsAllAfterPostGameReset } = require('./mp-review-done-split');
    const mobile = !!opts.mobile;
