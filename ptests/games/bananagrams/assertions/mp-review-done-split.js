 ...
 */
const { waitForPreSplitHand } = require('./mp-sync-split');
...
async function assertHostSplitSyncsAllAfterPostGameReset(ctx, lib, options = {}) {
    ...
    for (let i = 0; i < preSplit.length; i++) {
        ...
        if (s.gameStarted) {
            throw new Error(`${label}: ${role} gameStarted=true before host SPLIT (${JSON.stringify(s)})`);
        }
        if (s.postGameReview) {
            throw new Error(`${label}: ${role} still in postGameReview (${JSON.stringify(s)})`);
        }
        if (!s.canMutate) {
            throw new Error(`${label}: ${role} canMutatePlayingBoard must be true (${JSON.stringify(s)})`);
        }
    }
