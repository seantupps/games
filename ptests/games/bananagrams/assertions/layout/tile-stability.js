...
function assertExistingTilesStableAfterAction(before, after, label, options = {}) {
    ...
    if (movedWorld.length) {
        failWithSnapshot('assertion', [`${label}: existing tiles moved in world space (${JSON.stringify(movedWorld.slice(0, 12))})`], {});
    }
    if (movedScreen.length) {
        failWithSnapshot('assertion', [`${label}: existing tiles shifted on screen (${JSON.stringify(movedScreen.slice(0, 12))})`], {});
    }
    ...
    if (panDrift > maxPan || focalDrift > maxFocal || zoomDrift > maxZoom) {
        failWithSnapshot('assertion', [`${label}: viewport shifted (${JSON.stringify({ before: bvp, after: avp, panDrift, focalDrift, zoomDrift })})`], {});
    }
}
