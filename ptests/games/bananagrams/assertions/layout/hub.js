...
async function assertWinBannerLayout(page, label = 'win-banner') {
    ...
    try {
        ...
        failWithSnapshot('assertion', [`${label}: hub win banner not visible (${JSON.stringify(diag)})`], {});
    }
    try {
        ...
        failWithSnapshot('assertion', [`${label}: win banner layout (${JSON.stringify(snap)})`], {});
    }
}
...
async function assertBananagramsRackVisibleWithLowPersistedZoom(page, opts = {}) {
    ...
    if (!fit.ok) {
        failWithSnapshot('assertion', [`Bananagrams rack should auto-fit after low persisted zoom (${JSON.stringify(fit)})`], {});
    }
    if (typeof fit.zoom === 'number' && fit.zoom <= BAD_ZOOM + 0.01) {
        failWithSnapshot('assertion', [`Low persisted zoom was not corrected on mobile (zoom=${fit.zoom})`], {});
    }
}
