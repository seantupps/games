...
const { touchPanBackground } = require('../mobile/bananagrams_touch');
...
function assertPlacementExactMatch(refreshPl, resetPl, label) {
    if (refreshPl.dxFromCenter !== resetPl.dxFromCenter
        ...
        throw new Error(`${label}: rack placement must match refresh exactly `
            + `(refresh=${JSON.stringify(refreshPl)}, reset=${JSON.stringify(resetPl)})`);
    }
    if (refreshPl.rackCx !== resetPl.rackCx || refreshPl.rackCy !== resetPl.rackCy) {
        throw new Error(`${label}: rack screen center must match refresh exactly `
            + `(refresh=${JSON.stringify(refreshPl)}, reset=${JSON.stringify(resetPl)})`);
    }
}
...
async function assertMobileSoloSettingsResetMatchesRefresh(page, opts = {}) {
    ...
    if (!refreshPl.ok) {
        throw new Error(`${label}: refresh baseline failed (${JSON.stringify(refreshPl)})`);
    }
    if (!refreshPl.belowCenter) {
        throw new Error(`${label}: refresh rack should be below viewport center (${JSON.stringify(refreshPl)})`);
    }

    const pan = await touchPanBackground(frame);
    if (!pan.ok || (pan.dist ?? 0) < 20) {
        throw new Error(`${label}: could not pan board away before settings reset (${JSON.stringify(pan)})`);
    }
    await settleMobileViewport(page);
    ...
    if (!resetPl.ok) {
        throw new Error(`${label}: settings reset snapshot failed (${JSON.stringify(resetPl)})`);
    }
    if (!resetPl.belowCenter) {
        throw new Error(`${label}: settings reset should place rack below center `
            + `(refresh=${JSON.stringify(refreshPl)}, reset=${JSON.stringify(resetPl)})`);
    }
