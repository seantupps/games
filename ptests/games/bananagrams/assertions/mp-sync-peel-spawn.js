...
async function assertPeelSpawnVisibleAllPlayers(ctx, opts) {
    ...
    const beforeIdsList = opts.beforeIdsByPlayer || pages.map((_, i) => {
        ...
        throw new Error(`assertPeelSpawnVisibleAllPlayers: missing beforeIds for P${i + 1}`);
    });
    if (beforeIdsList.length !== pages.length) {
        throw new Error('assertPeelSpawnVisibleAllPlayers: beforeIdsByPlayer length mismatch');
    }
