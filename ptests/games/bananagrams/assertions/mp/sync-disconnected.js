...
const { readBoardField } = require('../core/capture');
...
async function assertHostPeelGuestDisconnectedTilesStable(opts) {
    ...
    if (!peelRes.peeled && peelRes.banner !== 'Peel!') {
        failWithSnapshot('assertion', [`Host peel trigger failed (${JSON.stringify(peelRes)})`], {});
    }
    ...
    if (disconnectedMoved.length) {
        failWithSnapshot('assertion', [`${label}: disconnected tiles moved (${JSON.stringify(disconnectedMoved)})`], {});
    }

    const addedIds = (guestAfter.tileIds || []).filter((id) => !(guestBefore.tileIds || []).includes(id));
    if (addedIds.length !== 1) {
        failWithSnapshot('assertion', [`${label}: expected exactly one new peel tile on guest (${JSON.stringify({ addedIds })})`], {});
    }
