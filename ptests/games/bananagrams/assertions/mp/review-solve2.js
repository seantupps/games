...
const { injectSnapshot, applyPlacements } = require('../../lib/ai-playthrough-apply');
const { awaitMpVictorySettled } = require('./victory');
const {
...
async function assertGuestWinAfterSolve2Placements(hostPage, guestPage, guestFrame, mp, label = 'solve-2 guest placements', meta = {}) {
    ...
    try {
        ...
        if (!solved.changed || !solved.placements?.length) {
            failWithSnapshot('assertion', [`solver could not connect straggler (stuck=${solved.stuck})`], {});
        }
    } catch (err) {
    ...
    try {
        ...
        if (!applied1.ok || !applied1.placed) {
            failWithSnapshot('assertion', [`applyPlacements returned ${JSON.stringify(applied1)}`], {});
        }
    ...
    try {
        ...
        if (!peelResult.claimed) {
            failWithSnapshot('assertion', [`_checkPeel returned false (${JSON.stringify(peelResult)})`], {});
        }
    } catch (err) {
    ...
    try {
        ...
        if (!solved.changed || !solved.placements?.length) {
            failWithSnapshot('assertion', [`solver stuck on final rack tile (stuck=${solved.stuck})`], {});
        }
    } catch (err) {
    ...
    try {
        ...
        if (!applied2.ok) {
            failWithSnapshot('assertion', [`applyPlacements returned ${JSON.stringify(applied2)}`], {});
        }
        await guestFrame.evaluate(() => {
            window.game._persistMpLayout?.();
            window.game.requestRender?.();
