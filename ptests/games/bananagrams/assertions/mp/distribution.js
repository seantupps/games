...
async function readTileDistributionState(frame, endingSnapshots = null) {
    return frame.evaluate((snaps) => {
        ...
        const resolvePlayerCount = () => {
            const party = typeof g._getPlayerUids === 'function' ? g._getPlayerUids().filter(Boolean) : [];
            if (party.length >= 2) return party.length;
            const fromBoard = board?.playerUids?.filter(Boolean)?.length;
            if (fromBoard >= 2) return fromBoard;
            const owned = board?.tilesOwnedByPlayer || g._mpOwned || {};
            const ownedN = Object.keys(owned).filter((u) => (owned[u]?.length || 0) > 0).length;
            if (ownedN >= 2) return ownedN;
            const pdN = Object.keys(room?.playerData || {}).length;
            if (pdN >= 2) return pdN;
            return 2;
        };
        ...
        const bagLabel = mode === 'multiplayer'
            ? (playerCount >= 3 ? 'banana-mp' : 'scrabble-mp')
...
async function assertTileDistributionInReview(frame, label = 'tile-distribution', opts = {}) {
    ...
    if (!inReview) {
        failWithSnapshot(label, ['expected post-game review before distribution check'], {});
    }
    ...
    if (!state.ok || state.mismatches?.length) {
        failWithSnapshot(label, ['tile distribution must match starting bag'], {
            bag: state.bagLabel,
            mode: state.mode,
...
async function probeTileDistributionSources(hostFrame, guestFrame, hostUid, guestUid) {
    const hostProbe = await hostFrame.evaluate(({ hostUid, guestUid }) => {
        const g = window.game;
        const rules = typeof BananaRules !== 'undefined' ? BananaRules : null;
        const cfg = rules ? rules.resolveBagConfig(new URLSearchParams(window.location.search)) : {};
        ...
        const playerCount = (() => {
            ...
            const ownedN = Object.keys(owned).filter((u) => (owned[u]?.length || 0) > 0).length;
            if (ownedN >= 2) return ownedN;
            const pdN = Object.keys(g.roomData?.playerData || {}).length;
            if (pdN >= 2) return pdN;
            return 2;
        })();
        const bag = rules ? rules.getTileBag('multiplayer', cfg, playerCount) : null;
        const frozen = g._mpFinalAuthoritySnapshot || null;
        const canonical = {
            ...(frozen?.canonical || {}),
            ...(g._mpCanonicalById || {})
