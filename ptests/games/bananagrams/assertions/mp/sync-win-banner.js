...
async function assertHubWinBannerVisibleSameTime(opts) {
    ...
    if (hostMs == null || guestMs == null) {
        ...
        failWithSnapshot(label, ['hub win banner visibility timeout'], { hostMs, guestMs, hostState, guestState });
    }

    const skew = Math.abs(hostMs - guestMs);
    if (skew > maxSkewMs) {
        failWithSnapshot(label, [`hub win banner skew ${skew}ms exceeds ${maxSkewMs}ms`], { hostMs, guestMs });
    }
