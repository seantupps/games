...
const { assertActionsWinBanner } = require('./review-actions');
...
async function assertHostWinBannerUpperViewport(hostPage, label = 'host win banner position') {
    ...
    if (!snap.visible) {
        failWithSnapshot('assertion', [`${label}: hub win banner not visible (${JSON.stringify(snap)})`], {});
    }
    if (snap.top > snap.maxTop) {
        failWithSnapshot(label, [`hub win banner too low on host (top=${snap.top}, max=${snap.maxTop})`], { snap });
    }
...
async function assertGuestWinHubBannerReview(hostPage, guestPage, guestFrame, label = 'guest win hub banner') {
    ...
    if (!hostState.visible || !guestState.visible) {
        failWithSnapshot(label, ['banner lost after review refit'], { hostState, guestState });
    }
