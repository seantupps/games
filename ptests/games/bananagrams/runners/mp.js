...
async function beforeLoop(page1OrPages, page2OrCtx, ctxMaybe = {}) {
    let pages;
    let ctx;
    if (Array.isArray(page1OrPages)) {
        pages = page1OrPages;
        ctx = page2OrCtx || {};
    } else {
        ctx = ctxMaybe;
        pages = ctx.pages?.length >= 2 ? ctx.pages : [page1OrPages, page2OrCtx];
    }
    ...
    for (let i = 0; i < scenarios.length; i++) {
        ...
        await runBananagramsMpAudit(pages, {
            ...
            rounds,
            pause: !!cfg.pause,
            playerDefs: ctx.playerDefs
        });
        if (i + 1 < scenarios.length) {
            await centerMpViewerOnPages(pages, { mobile: !!ctx.isMobile });
        }
    }
}
