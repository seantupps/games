...
const {
    ...
    readDealDiag,
    logVisAlways
} = require('../assertions/mp-board-visible');
const {
    assertScoreboard,
    assertAllPlayersScoreboard,
    hexToRgb
} = require('../assertions/mp-scoreboard');
...
function mpPages(allPages) {
    return allPages?.length ? { pages: allPages } : null;
}

async function captureAllPlayersState(pages, label) {
    return captureAllMpStates(pages, PLAYERS, label);
}
const SEQUENTIAL_GUEST_ORDERS = [[1, 2], [2, 1]];

async function assertJoinedPlayersReady(pages, playerIndices, roomId, label, opts = {}) {
    return assertJoinedPlayersReadyWithVisibility(pages, playerIndices, roomId, label, {
        ...opts,
        playerDefs: PLAYERS
    });
}
...
async function assertRefreshPreservesLayout(page, player, frame, tileId) {
    ...
    await waitForDiag(page, `${player.role} hand after reload`, ({ u }) => {
        const g = document.getElementById('game-frame')?.contentWindow?.game;
        return g
            && g.isMultiplayer
            && g.mode === 'multiplayer'
            ...
            && g.tiles?.length > 0
            && g._myUid?.() === u;
    }, { u: player.uid }, WAIT_MS, mpPages([page]));
...
async function runNonHostLeaveAudit(browser, options = {}) {
    ...
    try {
        ...
        await waitForDiag(page1, 'host purged P2 after leave', ({ gone }) => {
            const g = document.getElementById('game-frame')?.contentWindow?.game;
...
async function waitForDeal(page, uid, timeout = WAIT_MS, allPages = null) {
    await waitForDiag(page, `deal uid=${uid}`, ({ u }) => {
        ...
        return g
            ...
            && g.tiles?.length > 0
            && g._myUid?.() === u;
    }, { u: uid }, timeout, mpPages(allPages));
}
...
async function assertPartyBoardOnRtdb(hostPage, roomId, pages, label) {
    await waitForDiag(hostPage, `${label}: host _mpOwned`, ({ party }) => {
        const g = document.getElementById('game-frame')?.contentWindow?.game;
        return party.every((u) => (g._mpOwned?.[u]?.length || 0) > 0);
    }, { party: PLAYERS.map((p) => p.uid) }, WAIT_MS, mpPages(pages));
    try {
...
async function runMpMobileExtras3p(pages, frames, opts = {}) {
    ...
    const { runBananagramsMpMobileExtras } = require('../adapters/mobile-suite');
    const {
...
async function runBananagrams3pTest(browser, options = {}) {
    ...
    try {
        ...
        await assertAllPlayersScoreboard(frames, PLAYERS, zeroScores, 'Scoreboard');
        ...
        await waitPoolAll(buildMpCtx(PLAYERS, pages, { roomId }), pool);
        ...
        const { runMpAiPlaythrough3p, resetMpForAiPlaythrough3p } = require('./mp-audit/mp-ai-playthrough-3p');
        const { captureEndingLayoutFromFrame } = require('../assertions/mp-review');
        ...
        const {
            ...
            clickDone
        } = require('../scenarios/mp/review-mobile');
        const { waitMpResetAfterDone, assertMpPlayableAfterReset } = require('../assertions/mp-review');
        ...
        if (p3Mobile) {
            ...
            const { pushHostReviewStateToClients } = require('../assertions/mp-review');
    ...
    try {
        if (actionsOnly) {
            results.push(await runTimed('MP Bananagrams 3p', async () => {
                const { runBananagramsMpActionsAudit3p } = require('./mp-audit/actions-audit-3p');
