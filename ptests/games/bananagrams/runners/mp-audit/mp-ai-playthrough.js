...
async function tryPeel(ctx, snap, label) {
    ...
    if (snap.poolLen === 0) {
        ...
        if (ctx.assertWinBanner) {
            const { assertHubWinBannerVisibleSameTime } = require('../../assertions/mp-sync-win-banner');
...
async function assertActionsWinBanner(pages, label = 'actions win banner') {
    const { isHubWinBannerDomVisible } = require('../../assertions/mp-sync-win-banner');
...
async function awaitMpActionsReviewOpen(page1, page2, { mobile = false } = {}) {
    ...
    const { waitPostGameReview } = require('../../scenarios/mp/review-mobile');
    if (isMpHeaded()) {
...
async function exitReviewAfterActionsSession(page1, page2, frame1, frame2, mp, label) {
    ...
    const { clickDone } = require('../../scenarios/mp/review-mobile');
