...
const mpActionAssertions = require('../../../shared/assertions/mp-authority');
...
const { assertActionBannerOnBoth, assertActionBannerAllPlayers } = mpActionAssertions;
const {
...
/**
 * Wait until every client sees the same pool count.
...
/** @deprecated use waitPoolAll(ctx) */
...
/**
 * Generalized join readiness — 2p uses BANANA_2P_PLAYERS; pass ctx.players for 3p+.
 * @param {import('playwright').Page[]} pages
 * @param {number[]} playerIndices
 * @param {object} playerDefs — array of { uid, role }
 */
...
async function assertJoinedPlayersReady2p(pages, playerIndices, roomId, label, opts = {}) {
    return assertJoinedPlayersReady(pages, playerIndices, roomId, label, {
        ...opts,
