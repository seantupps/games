/**
 * Sequential MP join readiness — deal sync + board visibility (3p+).
 */
const { STEP_MS } = require('../../../shared/infra/timeouts');
const { assertJoinedPlayersReady } = require('./mp-state');
const { assertBoardVisible, logVisAlways } = require('../assertions/mp-board-visible');
...
async function assertJoinedPlayersReadyWithVisibility(pages, playerIndices, roomId, label, opts = {}) {
    ...
    await assertJoinedPlayersReady(pages, playerIndices, roomId, label, {
        ...opts,
        playerDefs
    });
