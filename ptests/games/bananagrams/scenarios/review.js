...
const { runLastBunchPeelTests } = require('./last-bunch');
const {
    runSolveAndAssert,
    assertMpSolveSynced,
    solveExpectations,
...
const {
    ...
    collectMpClientDiag
} = require('../assertions/bananagrams_solve2_guest_finish_assertions');
const {
    assertGuestWinHubBannerReview
} = require('../assertions/bananagrams_mp_guest_win_banner_assertions');
const mpLib = require('../lib/mp-lib');
...
async function runReviewScenarioAudit(page1, page2, options = {}) {
    ...
    let frame2;
    if (options.skipSeed) {
        const { resetMpForAiPlaythrough } = require('../desktop-mp/audit/mp-ai-playthrough');
        frame1 = await getGameFrame(page1);
        frame2 = await getGameFrame(page2);
