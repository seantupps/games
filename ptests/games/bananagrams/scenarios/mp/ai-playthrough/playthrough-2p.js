...
} = require('../../../assertions/mp/distribution');
const { readBoardField } = require('../../../assertions/core/capture');
const { assertPoolBothStrict, assertPoolBothNatural } = require('../../../assertions/mp/sync');
const {
    injectSnapshot,
    applyPlacements,
    snapshotMpAiState,
...
async function tryPeel(ctx, snap, label) {
    ...
    if (snap.poolLen === 0) {
        ...
        if (ctx.assertWinBanner) {
            const timing = await syncWinBanner.assertHubWinBannerVisibleSameTime({
                page1: hostPage,
                page2: page,
    ...
    if (isLastBunchPeel) {
        await assertPoolBothStrict(hostPage, page, poolBefore, `${label} pre last-bunch peel`);
    }
    ...
    if (isLastBunchPeel) {
        await assertPoolBothNatural(hostPage, page, poolAfter, `${label} post last-bunch peel`, 150);
    }
 ...
 */
async function runMpAiPlaythrough2p(opts) {
    if (process.env.FIVE_MP_ACTIONS_DEBUG === '0') {
        process.env.BANANA_AI_QUIET = '1';
    }
    const ctx = resolvePlaythroughCtx(opts);
    attachFramesToCtx(ctx, opts.frames ?? (opts.frame1 && opts.frame2 ? [opts.frame1, opts.frame2] : ctx.frames));
    const { page1, page2, frame1, frame2, mp, mobile: ctxMobile } = legacyPagesFromCtx(ctx);
    ...
    const {
        ...
        maxRoundTrips = resolveMpAiMaxRoundTrips(playToWin, { playFallback: 30, auditFallback: 30 }),
    ...
    const maxRounds = !mpAiRoundTripCapEnabled()
        ? Number.POSITIVE_INFINITY
        : (playToWin && assertActionsWinInvariants)
 ...
 */
async function resetMpForAiPlaythrough2p(opts) {
    const ctx = resolvePlaythroughCtx(opts);
    attachFramesToCtx(ctx, opts.frames ?? (opts.frame1 && opts.frame2 ? [opts.frame1, opts.frame2] : ctx.frames));
    const { page1, page2, frame1, frame2, mp, mobile: ctxMobile } = legacyPagesFromCtx(ctx);
    const {
...
async function exitReviewAfterActionsSession(page1, page2, frame1, frame2, mp, label) {
    ...
    const { clickDone } = require('../review-mobile');
...
async function runMpAiActionsOnly2p(page1, page2, options = {}) {
    const ctx = options.ctx ?? resolvePlaythroughCtx({ page1, page2, mobile: options.mobile });
    preloadAiDictionary();
    ...
    let frame2;
    if (!options.skipSeed) {
        ({ frame1, frame2 } = await bootMpPlaySessionFromPages(page1, page2, { mobile }));
        await probeHeadedMobileViewport([page1, page2], 'after boot session', mobile);
    } else {
...
async function runMpAiPlaythroughFromCtx(ctx, opts = {}) {
    return runMpAiPlaythrough({ ...opts, ctx });
}

async function resetMpForAiPlaythroughFromCtx(ctx, opts = {}) {
    return resetMpForAiPlaythrough({ ...opts, ctx });
}
...
async function advanceActionsRoundAfterReviewFromCtx(ctx, roundLabel, options = {}) {
    const { page1, page2, mp, frame1, frame2, mobile } = legacyPagesFromCtx(ctx);
    return advanceActionsRoundAfterReview(
        page1, page2, frame1, frame2, mp, options.mobile ?? mobile, roundLabel, options
    );
}

async function finishPausedReviewSessionFromCtx(ctx, options = {}) {
    const { page1, page2, mobile } = legacyPagesFromCtx(ctx);
    return finishPausedReviewSession(page1, page2, { mobile: options.mobile ?? mobile });
}

async function exitReviewAfterActionsSessionFromCtx(ctx, label) {
    const { page1, page2, mp, frame1, frame2 } = legacyPagesFromCtx(ctx);
    return exitReviewAfterActionsSession(page1, page2, frame1, frame2, mp, label);
}

function resolveMpAiPlayerCount(opts) {
    if (opts.playerCount != null) return opts.playerCount;
    if (opts.pages?.length) return opts.pages.length;
    if (opts.page1 && opts.page2) return 2;
    return 2;
}

async function runMpAiPlaythrough(opts) {
    const n = resolveMpAiPlayerCount(opts);
    if (n >= 3) {
        const { runMpAiPlaythroughN } = require('./playthrough-n');
        return runMpAiPlaythroughN(opts);
    }
    return runMpAiPlaythrough2p(opts);
}

async function resetMpForAiPlaythrough(opts) {
    const n = resolveMpAiPlayerCount(opts);
    if (n >= 3) {
        const { resetMpForAiPlaythroughN } = require('./playthrough-n');
        return resetMpForAiPlaythroughN(opts);
    }
    return resetMpForAiPlaythrough2p(opts);
}

async function runMpAiActionsOnly(page1, page2, options = {}) {
    if ((options.playerCount ?? 0) >= 3) {
        throw new Error('runMpAiActionsOnly(page1,page2): use runMpAiActionsOnlyN(browser) for 3p+');
    }
    if (options.ctx) {
        return runMpAiActionsOnlyFromCtx(options.ctx, options);
    }
    return runMpAiActionsOnly2p(page1, page2, options);
}

module.exports = {
    runMpAiPlaythrough,
    runMpAiPlaythroughFromCtx,
    runMpAiActionsOnly,
    runMpAiActionsOnlyFromCtx,
    resetMpForAiPlaythrough,
    resetMpForAiPlaythroughFromCtx,
    injectSnapshot,
    applyPlacements,
    solveAndApplyAiMove,
    readPeelSeq: (page) => readBoardField(page, 'peelSeq'),
    resolvePlayToWin,
    resolveWinSteering,
    resolveDesiredWinSide,
