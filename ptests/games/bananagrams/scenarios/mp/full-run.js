...
const { assertDumpSpawnQuick } = require('../../assertions/dump-spawn');
const { solveAndApplyAiMove } = require('../../runners/mp-audit/mp-ai-playthrough');
...
async function runFullScenario(scenarioCtx) {
    ...
    const mpCtx = scenarioCtx.ctx || buildMpCtx2p(page1, page2, { roomId, mobile, options });
    if (mpCtx.playerCount >= 3) {
        const { runFullScenario3p } = require('./full-run-3p');
        return runFullScenario3p(scenarioCtx);
    }

    const pages = mpCtx.pages;
    let frame1;
    let frame2;
    let poolAfterDeal;
    ...
    if (focusDumpPeel) {
        ...
        if (!skipSeed) {
            ...
            const reset = await resetMpForAiPlaythrough({ page1, page2, frame1, frame2, mp, mobile });
            frame1 = reset.frame1;
            frame2 = reset.frame2;
        }
    ...
    if (!mobile) {
        const { assertGuestFirstSplitStableAfterReset } = require('../../assertions/mp-sync-split');
        ...
        await waitForDeal(page1, 'P1', mp);
        await waitForDeal(page2, 'P2', mp);
        const { waitForPreSplitHand } = require('../../assertions/mp-sync-split');
        await Promise.all([
    ...
    await lib.waitPoolAll(mpCtx, poolAfterDeal);
